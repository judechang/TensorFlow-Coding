"""
Yanggang.hang
TensorFlow高级实践第一课：TensorFlow编程基础入门
"""
...
class cnnModel(object):
    # 返回一个cnnModel 其中可定义参数为percent,learning_rate,learning_rate_decay_factor
    ...
    def __init__(self,percent,learning_rate,learning_rate_decay_factor):
        ...
        def create_conv_layer(input_data, filter_size, num_filters):
            ...
            filters = tf.Variable(tf.truncated_normal(shape=(
                filter_size, filter_size, tf.cast(input_data.shape[-1], dtype=tf.int32), num_filters),
                                                                     stddev=0.05))
            # tf.truncated_normal将进行随机的正交初始化 tf.cast重新定义数据类型
        ...
        def create_CNN(input_data, num_classes, keep_prop):
            ...
            fc_resultl = fc_layer(flattened_layer=flattened_layer,
                                  ...
                                  num_outputs=200)
            # Second fully connected layer accepting the output of the previous fully connected layer. Number of outputs
            #  is equal to the number of dataset classes.
        ...
        # 将label变成one-hot编码，因为softmax_propabilities是一个数组，是10个概率，每个概率代表着预测结果属于其index类
        # 的概率，为了计算交叉熵，我们需要把label也转换成一个数组
    ...
    def step(self, sess, shuffled_data, shuffled_labels, graph, forward_only=None):
        ...
        if forward_only:
            ...
            print(dataset_array)
            feed_dict_test = {data_tensor: dataset_array,keep_prop: 0.5
            }
            softmax_propabilities_, softmax_predictions_ = sess.run([self.softmax_propabilities,
                                                                     self.softmax_predictions], feed_dict=feed_dict_test)
